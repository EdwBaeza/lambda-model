...
```bash
lambda-model$ sam build --use-container
```
...
```bash
lambda-model$ sam logs -n LambdaModelFunction --stack-name lambdamodelfunction --tail
```
...
```bash
lambda-model$ docker build . -f tests/Dockerfile -t lambda-model:tests
lambda-model$ docker run --rm lambda-model:tests python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name lambda-model
```
